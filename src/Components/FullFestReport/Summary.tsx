...
const Summary: FC<{
  ...
  const getFilteredTypes = (
    data: SubjectSummaryItem[] | undefined,
  ...
  const aiSections = [
    {
      ...
      range: "0% - 20%",
      items: [...getFilteredTypes(SubWise?.["Very Weak"])],
    },
    {
      title: "Weak",
      color: "bg-danger",
      range: "20% - 40%",
      items: [...getFilteredTypes(SubWise?.Weak)],
    },
    {
      title: "Average",
      color: "bg-warning",
      range: "40% - 60%",
      items: [...getFilteredTypes(SubWise?.Average)],
    },
    {
      title: "Strong",
      color: "bg-success",
      range: "60% - 80%",
      items: [...getFilteredTypes(SubWise?.Strong)],
    },
    {
      title: "Very Strong",
      color: "bg-success-light",
      range: "80% - 100%",
      items: [...getFilteredTypes(SubWise?.["Very Strong"])],
    },
  ];
  ...
  const Sections = [
    {
      ...
      color: "bg-danger-dark",
      range: "0% - 20%",
      items: AttemptingStrategyWise?.["Very Weak"]?.map(
    ...
    },
    {
      ...
      color: "bg-danger",
      range: "20% - 40%",
      items: AttemptingStrategyWise?.Weak?.map((item) => item?.subjectName),
    },
    {
      title: "Average",
      color: "bg-warning",
      range: "40% - 60%",
      items: AttemptingStrategyWise?.Average?.map((item) => item?.subjectName),
    },
    {
      title: "Strong",
      color: "bg-success",
      range: "60% - 80%",
      items: AttemptingStrategyWise?.Strong?.map((item) => item?.subjectName),
    },
    {
      title: "Very Strong",
      color: "bg-success-light",
      range: "80% - 100%",
      items: AttemptingStrategyWise?.["Very Strong"]?.map(
        (item) => item?.subjectName
      ),
    },
  ];
  const currentSections = tabIndex === 0 ? Sections : aiSections;
  const formatType = (str: string | undefined) => {
    return str
      ?.replace(/([A-Z])/g, " $1")
      .trim()
  ...
  return (
    <div>
      ...
      <div className="pt-5">
        <Tabs
          className="horizontal-tabs xl:w-1/2 mx-auto"
