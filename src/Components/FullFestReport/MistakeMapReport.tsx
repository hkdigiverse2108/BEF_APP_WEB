...
const MistakeMapReport: FC<{ MistakeMapReport: MistakeMapReportType[] }> = ({ MistakeMapReport }) => {
  console.log(MistakeMapReport);
        ...
        <div className="grid gap-4 grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-5 w-full">
          {MistakeMapReport?.map((item, i) => (
            <div key={i} className="rounded-lg shadow-lg bg-white">
              <div className={`px-4 py-2 rounded-t-lg text-lg text-white flex justify-between capitalize ${i === 0 ? "bg-danger-dark" : i === 1 ? "bg-danger" : i === 2 ? "bg-warning" : i === 3 ? "bg-success" : "bg-success-light"}`}>
                <h3>{item?.whyFalse}</h3>
                <p>{item?.total}</p>
              </div>
