...
const YoutubeValues = () => {
  ...
  return (
    <div className="pb-5">
      ...
      <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">{isLoading ? [...Array(3)].map((_, i) => <Skeleton.Node key={i} active style={{ width: "100%", height: 300, borderRadius: 5 }} />) : YoutubeValuesData?.map((video: { thumbnail: string; title: string ,link: string}, index: number) => <YouTubeCard key={index} videoId={video.link} title={video.title} thumbnail={video.thumbnail} />)}</div>
    </div>
  );
};
