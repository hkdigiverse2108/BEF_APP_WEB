        ...
        <div className="border-y border-card-border p-4 my-5 ">
          <div className="flex flex-row max-sm:flex-col items-center sm:gap-4 w-full h-full">
            {loading ? (
              ...
              <>
                <img className="object-cover w-16 h-16 sm:w-12 sm:h-12 rounded-full border-2 border-white" src={contest?.subjectId?.image} alt={contest?.subjectId?.name} />
                <div className="grid gap-1 w-full">
                  <h3 className="text-lg max-sm:text-center text-left font-medium tracking-tight">{contest?.subjectId?.name}</h3>
                </div>
              </>
            )}
          </div>
