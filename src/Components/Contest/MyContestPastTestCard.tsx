...
const MyContestPastTestCard: FC<ContestDetailCardProps> = ({ contestData }) => {
  ...
  return (
    <div onClick={() => navigate(`${ROUTES.EXAM.RESULT}?qaFilter=${_id}&contestFilter=${contestId}`)} className="border border-primary-light rounded-xl overflow-hidden capitalize flex flex-col justify-between cursor-pointer">
      {/* Header */}
