...
const MyContestUpcomingCard: FC<ContestDetailCardProps> = ({ contestData }) => {
  ...
  return (
    <>
              ...
              <div className="flex flex-col gap-1 py-1 text-sm font-semibold">
                ...
                <div className="flex justify-between items-center mt-2 gap-2">
                  <section onClick={(e) => handleJoin(e)}>
                    <p className="font-semibold text-base bg-success text-white px-6 py-1 w-fit rounded cursor-pointer hover:opacity-90">{startingAttempt ? "Loading..." : contestData.status === "ongoing" ? "Resume" : "Join"}</p>
                  </section>
                  {((contestData as any).result === true || contestData.status === "completed") && (
                    <section onClick={(e) => handleResult(e)}>
                      <p className="font-semibold text-base bg-primary text-white px-6 py-1 w-fit rounded cursor-pointer hover:opacity-90">{checkingAttempts ? "Loading..." : "Result"}</p>
                    </section>
                  )}
                </div>
              </div>
                ...
                <div className="flex justify-between text-sm font-normal">
                  ...
                  <div className="flex items-center gap-2">
                    {contestData.status !== "completed" && (
                      <section onClick={(e) => handleJoin(e)}>
                        <p className="font-semibold text-lg bg-success text-white px-6 py-1 w-fit rounded cursor-pointer hover:opacity-90">{contestData.status === "ongoing" ? "Resume" : "Join"}</p>
                      </section>
                    )}
                    {(contestData.status === "completed" || (contestData as any).result === true) && (
                      <section onClick={(e) => handleResult(e)}>
                        <p className="font-semibold text-lg bg-primary text-white px-6 py-1 w-fit rounded cursor-pointer hover:opacity-90 text-center">{checkingAttempts ? "Loading..." : "Result"}</p>
                      </section>
                    )}
                  </div>
                </div>
      ...
      <ReattemptModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} contest={contestData} contestId={_id} />
    </>
  );
};
