...
const ContestFilterDrawer = () => {
  ...
  const [localFilters, setLocalFilters] =
    useState<ContestFilters>(isContestFilters);

  const updateFilter = <K extends keyof ContestFilters>(
    key: K,
  ...
  const { data: ContestTypeData, isLoading } = useGetApiQuery(
    { url: URL_KEYS.CONTEST.TYPE.TYPE },
    { skip: false }
  );
    ...
    <div>
      ...
      <ul className="flex flex-wrap gap-3 pt-3">
        {data?.map((item, i) => (
          <li
            key={i}
            onClick={() => updateFilter(key, item)}
            className={`border py-2 px-3 w-fit rounded-full cursor-pointer transition-colors duration-200 text-theme bg-input-box ${
      ...
      {isLoading ? (
        ...
        <ul className="flex flex-wrap gap-3 pt-3">
          {data?.map((item, i) => (
            <li
              key={i}
              onClick={() => updateFilter(key, item.name)}
              className={`border py-2 px-3 w-fit rounded-full cursor-pointer transition-colors duration-200 text-theme bg-input-box ${
                selected === item.name
                  ? "border-primary !text-primary !bg-bg-light"
                  : "border-box-border hover:border-theme"
  ...
  return (
    <Drawer
      ...
      placement="right"
      size="default"
      onClose={() => dispatch(setContestFilterDrawer({ open: false }))}
      open={isContestFilterDrawer.open}
    ...
    >
      <div className="flex flex-col items-start">
        ...
        {renderStringSection(
          "Contest Type",
          "contestType",
          ContestTypeData?.data || [],
          localFilters.contestType
        )}
