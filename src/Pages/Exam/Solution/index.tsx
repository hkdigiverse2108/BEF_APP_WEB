...
const Solution = () => {
  const { SolutionFilter, SolutionCurrentQuestion } = useAppSelector(
    (state) => state.filter
  );
  ...
  const QuestionData = QAData?.answers?.find(
    (q) => q._id === currentQuestionAnswers?._id
  );
  ...
  const { data: AiMentorData } = useGetApiQuery(
    currentQuestion?._id
      ? {
          url: `${URL_KEYS.QUESTION.AI_MENTOR}?questionId=${currentQuestion?._id}`,
        }
      : { skip: true }
  );
  ...
  const handleSolutionsFilter = (values: string) => {
    ...
    if (values === "all") setQaAnswers(QaAnswers);
      ...
      setQaAnswers(
        QaAnswers?.filter((item) => CheckRightAndWrongAnswers(item) === values)
      );
  };
  useEffect(() => {
    if (SolutionFilter === "all") setQaAnswers(originalAnswers);
      ...
      setQaAnswers(
        originalAnswers.filter(
          (item) => CheckRightAndWrongAnswers(item) === SolutionFilter
        )
      );
  ...
  const handleWhyFalseChange = async (value: string) => {
    const enriched = {
      ...
      answers: QaAnswers?.map((ans: Answer) =>
        ans.questionId === currentQuestionAnswers?.questionId
          ? { ...ans, whyFalse: value }
          : ans
      ),
    };
  ...
  const ALMentor = (className: string) => (
    <span
      className={`${className} flex gap-2 bg-input-box font-semibold text-sm p-2 px-3 rounded capitalize`}
    >
      <Tooltip
        title={
          "AL Mentor: Unlock strategies by exploring how others tackled this question and discover your most effective weapon bases on past performance!"
        }
      >
        ...
        <div className="grid grid-cols-1 2xl:grid-cols-6 gap-6 w-full">
          ...
          <div className="col-span-4 2xl:col-span-4">
            ...
            <div>
              <div className="flex flex-wrap justify-between items-center gap-3 mt-3">
                <div className="flex flex-wrap items-center gap-3">
                  <span
                    className="bg-input-box font-semibold text-sm p-2 rounded cursor-pointer"
                    onClick={() => setOpenQuestion(!isOpenQuestion)}
                  >
                  ...
                  <Select
                    ...
                    className="!m-0"
                    onChange={handleSolutionsFilter}
                    defaultValue={SolutionFilter}
                  />
                </div>
                <Link
                  to={`${ROUTES.EXAM.MISTAKE_MAP_REPORT.replace(
                    ":id",
                    id || ""
                  )}${search}`}
                  className="bg-linear-to-r from-(--primary) to-(--success) text-white font-semibold text-sm p-2 px-4 rounded capitalize"
                >
                  mistake map report
                </Link>
                  ...
                  <div className="flex flex-wrap items-center gap-3 mt-3">
                    <div className="relative inline-block">
                      <span className="bg-input-box font-semibold text-sm p-2 px-4 rounded">
                        Question : {currentQuestionAnswers?.qaNumber}
                      </span>
                      {(() => {
                        const currentStack = CheckIsStackNumber(
                          QAData?.positiveMarks as number
                        );
                    ...
                    {CheckRightAndWrongAnswers(currentQuestionAnswers) ===
                      "correct" && (
                      <span className="bg-green-100 text-green-700 text-sm font-semibold py-2 px-4 rounded">
                        +{" "}
                        {CheckIsStackNumber(QAData?.positiveMarks as number) ||
                          0}
                      </span>
                    )}
                    {CheckRightAndWrongAnswers(currentQuestionAnswers) ===
                      ...
                      <span className="bg-red-100 text-red-700 text-sm font-semibold py-2 px-4 rounded">
                        {CheckIsStackNumber(QAData?.negativeMarks as number) ||
                          0}
                      </span>
                    )}
                    <div className="flex flex-wrap items-center justify-center sm:ml-auto gap-3">
                      <span
                        ...
                        className={`flex gap-2 bg-input-box font-semibold text-sm p-2 px-4 rounded capitalize ${
                          language === "hindiQuestion"
                            ? "border border-input-box-dark"
                            : "border border-input-box"
                        }`}
                      >
                      ...
                      {(CheckRightAndWrongAnswers(currentQuestionAnswers) ===
                        "incorrect" ||
                        (currentQuestionAnswers?.type === "fearDriverSkip" &&
                          !CheckWhyFalseAnswers(currentQuestionAnswers))) && (
                        <Select
                          allowClear
                          loading={isPostLoading}
                          onChange={handleWhyFalseChange}
                          placeholder="Why False"
                          options={WhyFalseOptions}
                          className="!m-0 solution-select"
                          value={currentQuestionAnswers?.whyFalse}
                        />
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <p className="bg-input-box font-semibold text-sm p-2 px-4 rounded text-neutral-500">
                      ...
                      <span className="text-black">
                        {" "}
                        {formatType(currentQuestionAnswers?.type || "")}
                      </span>
                    </p>
                    {ALMentor("sm:hidden")}
                    <div className="flex flex-wrap items-center justify-center sm:ml-auto gap-3">
                  ...
                  <div className="relative my-6 shadow-lg rounded-2xl p-0.5 animate-gradient-border bg-linear-to-r from-(--primary) via-(--success) to-(--primary) bg-[length:200%_200%]">
                    <div className="bg-white rounded-2xl p-6">
                      ...
                      <ul className="list-[square] ps-5">
                        ...
                        {AiMentor?.otherStrategy?.strategyPercentage &&
                          AiMentor?.otherStrategy?.strategyType && (
                            <li className="font-normal mb-2">
                              Other :{" "}
                        ...
                        {AiMentor?.youStrategy?.percentage &&
                          AiMentor?.youStrategy?.type && (
                ...
                <div className="bg-white border border-card-border p-3 sm:p-6 rounded-xl shadow-lg">
                      ...
                      <div className="mb-4">
                        {isLoading ? (
                          <Skeleton.Input
                            ...
                            block
                          />
                        ) : isImage(currentQuestionLanguage?.question || "") ? (
                          <img
                            src={currentQuestionLanguage?.question}
                            alt="question"
                      ...
                      {QaAnswers.length > 0 &&
                        (currentQuestion?.questionType === QUE_TYPE.STATEMENT ||
                          ...
                          <div className="space-y-6 pb-6 rounded-2xl">
                            ...
                            <span className="font-normal text-lg rounded">
                        ...
                        currentQuestion?.questionType === QUE_TYPE.PAIR && (
                          <div className="space-y-4 pb-6 rounded-2xl">
                            <PairTable
                              ...
                              pairTitle={
                                currentQuestionLanguage?.pairQuestion?.[0]
                                  ?.combined || ""
                              }
                            />
                            <span className="font-normal text-lg rounded">
                  ...
                  <div className="!grid grid-cols-1 lg:grid-cols-2 gap-3">
                    {isQaLoading
                      ...
                      : Object.keys(
                          currentQuestionLanguage?.options || {}
                        )?.map((opt, i) => {
                          const userAnswer = currentQuestionAnswers?.answer;
                          const hasAnswered = !!userAnswer;
                          const isRightAnswer =
                            hasAnswered &&
                            opt === currentQuestionAnswers?.rightAnswer;
                          const isWrongAnswer =
                            hasAnswered &&
                            opt === userAnswer &&
                            opt !== currentQuestionAnswers?.rightAnswer;
                          ...
                          return (
                            <div
                              key={i}
                              className={`border-1 border-card-border flex items-center gap-3 p-4 m-0 rounded-md cursor-pointer transition-all ${
                                isRightAnswer
            ...
            <div className="flex flex-wrap justify-between gap-2 ">
              ...
              {!(currentQuestionNumber === isQaAnswers.length) && (
                <div
                  className={`${
                    currentQuestionNumber === 1 && " w-full flex justify-end"
                  } `}
                >
                  <FormButton
                    onClick={handleNextQueClick}
                    disabled={currentQuestionNumber === isQaAnswers.length}
                    text="Next"
                    className="custom-button w-full sm:w-40 button button--mimas text-center !p-4 !h-13 uppercase"
                  />
                </div>
          ...
          <div
            onClick={() => setOpenQuestion(!isOpenQuestion)}
            className="rounded-2xl w-full hidden before:fixed before:bg-black before:opacity-40 before:inset-0 before:z-50"
            style={{ display: isOpenQuestion ? "block" : "none" }}
          >
            <div
              onClick={(e) => e.stopPropagation()}
              className="2xl:gap-x-10 space-y-3 fixed bg-[#ffffff] w-full max-w-[400px] min-w-[200px] top-0 right-0  px-5 py-4 h-full shadow-md overflow-auto z-50"
            >
              <div className="mb-6">
              ...
              {isQaAnswers?.length !== 0 ? (
                <div className="grid grid-cols-5 gap-2">
                  {isQaAnswers?.map((item, i) => {
                    let type: "answered" | "unanswered" | "not-visited" =
                      "not-visited";
                    const isSkipped =
                      item?.type === "skip" || item?.type === "fearDriverSkip";
