    ...
    <div className="sub-container pt-4 md:pt-8 result">
      ...
      <div className="flex justify-start mt-3">
        <button
          ...
          className="2xl:hidden cursor-pointer p-1 flex justify-center items-center rounded-xl w-10 sm:w-12 h-10 sm:h-12 bg-input-box"
        >
        ...
        <div className="flex flex-col md:flex-row 2xl:gap-7">
          <div className="!sticky !top-32 !self-start max-2xl:z-50">
            <div
              ...
              className="max-2xl:hidden 2xl:!flex 2xl:items-start max-2xl:before:fixed max-2xl:before:bg-black max-2xl:before:opacity-40 max-2xl:before:inset-0 max-2xl:before:z-50"
              style={{ display: isOpen ? "block" : "none" }}
            >
              <div
                onClick={(e) => e.stopPropagation()}
                className=" p-6 2xl:gap-x-10 max-2xl:space-y-3 max-2xl:fixed 2xl:w-100 rounded-xl bg-input-box max-2xl:bg-[#ffffff] max-2xl:top-0 max-2xl:right-0 max-2xl:px-5 max-2xl:py-4 max-2xl:h-full max-2xl:shadow-md max-2xl:overflow-auto max-2xl:z-50"
              >
          ...
          <div className="tab-panels w-full">
            ...
            <div hidden={tabIndex !== 1}>
              <AiPowered
                data={OverviewData?.qaTypeMetrics}
                subjectName={ContestSubject?.name}
              />
            </div>
            ...
            <div hidden={tabIndex !== 3}>
              <CompareCompetitor
                data={compareWithCompetitorData}
                subjectName={ContestSubject?.name}
              />
            </div>
            <div hidden={tabIndex !== 4}>
              <EliminationSkill
                data={eliminationReportTypeData}
                subjectName={ContestSubject?.name}
              />
            </div>
