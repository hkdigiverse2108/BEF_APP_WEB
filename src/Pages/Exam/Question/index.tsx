            ...
            <div className="bg-input-box p-3 sm:p-6 rounded-2xl">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2 my-8 max-xl:justify-center">
                {[
                  ...
                  { label: "Logic Play", color: "bg-rose-500", icon: <FaLightbulb className="text-lg" /> },
                  { label: "Intuition Hit", color: "bg-sky-500", icon: <MdVisibility className="text-lg" /> },
                  { label: "Blind Fire", color: "bg-amber-500", icon: <MdVisibilityOff className="text-lg" /> },
                  { label: "Skip", color: "bg-purple-700", icon: <TbMessageQuestion className="text-lg" /> },
                  { label: "Fear - Driver Skip", color: "bg-green-700", icon: <RiFileCheckFill className="text-lg" /> },
                ].map((btn, i) => (
                  <button key={i} className={`flex justify-center items-center gap-3 shadow-btn-shadow color-1 p-3 text-sm font-semibold text-white rounded-xl transition-all duration-200 hover:animate-pulse ${btn.color}`}>
                    {btn.icon}
                    {btn.label}
                  </button>
                ))}
